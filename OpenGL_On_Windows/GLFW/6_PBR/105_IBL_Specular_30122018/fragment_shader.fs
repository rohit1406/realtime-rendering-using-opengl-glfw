...
// IBL
uniform samplerCube u_sampler_irradianceMap;
uniform samplerCube u_sampler_prefiltered_map;
uniform sampler2D u_sampler_brdf_LUT;
//lights
...
void main()
{
	...
	vec3 kD = 1.0 - kS;
	kD *= 1.0 - metallic;
	vec3 irradiance = texture(u_sampler_irradianceMap, N).rgb;
	vec3 diffuse = irradiance * albedo;
	
	const float MAX_REFLECTION_LOD = 4.0;
	vec3 prefilteredColor = textureLod(u_sampler_prefiltered_map, R, roughness * MAX_REFLECTION_LOD).rgb;
	
	vec2 envBRDF  = texture(u_sampler_brdf_LUT, vec2(max(dot(N, V), 0.0), roughness)).rg;
