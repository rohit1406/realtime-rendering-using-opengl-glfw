...
unsigned int irradianceMapCM;
unsigned int prefilteredEnvironmentMapCM;
...
int main()
{
	...
	Shader irradianceShader("convoluted_irradiance.vs", "convoluted_irradiance.fs");
	Shader prefilteredShader("prefiltered_convolution.vs", "prefiltered_convolution.fs");
	Shader brdfShader("precomputing_brdf.vs", "precomputing_brdf.fs");
	...
	*/
	...
	unsigned int albedo = loadTexture(
		"resources/textures/pbr/copper/Copper-scuffed_basecolor-boosted.png", //path
		false //gamma correction
	);
	unsigned int metallic = loadTexture(
		"resources/textures/pbr/copper/Copper-scuffed_metallic.png", //path
		false //gamma correction
	);
	unsigned int normal = loadTexture(
		"resources/textures/pbr/copper/Copper-scuffed_normal.png", //path
		false //gamma correction
	);
	unsigned int roughness = loadTexture(
		"resources/textures/pbr/copper/Copper-scuffed_roughness.png", //path
		false //gamma correction
	);
	unsigned int ao = loadTexture(
		"resources/textures/pbr/copper/rustediron2_ao.png", //path
		false //gamma correction
	);
	...
	glEnable(GL_TEXTURE_CUBE_MAP_SEAMLESS); //to properly filter across cube map faces
	...
	lighting_shader.setInt("u_sampler_irradianceMap", 5);
	lighting_shader.setInt("u_sampler_prefiltered_map", 6);
	lighting_shader.setInt("u_sampler_brdf_LUT", 7);
	...
	*/
	...
	glGenTextures(1, &irradianceMapCM);
	glBindTexture(GL_TEXTURE_CUBE_MAP, irradianceMapCM);
	for (unsigned int i = 0; i < 6; i++)
	{
	...
	//re-scale capture framebuffer to new resolution
	glBindFramebuffer(GL_FRAMEBUFFER, captureFBO); //bind fbo
	glBindRenderbuffer(GL_RENDERBUFFER, captureRBO); //bind rbo
	...
	//pass uniforms
	cubeShader.setInt("u_sampler_convoluted_environment_map", 0);
	cubeShader.setMat4("u_perspective_matrix", gCaptureProjection);
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_CUBE_MAP, envCubeMapCM);
	...
	{
		cubeShader.setMat4("u_view_matrix", gCaptureViews[i]);
		glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, irradianceMapCM, 0);
		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
		//draw cube
		glBindVertexArray(cubeVAO);
		glDrawArrays(GL_TRIANGLES, 0, 36);
	...
	glGenTextures(1, &prefilteredEnvironmentMapCM);
	glBindTexture(GL_TEXTURE_CUBE_MAP, prefilteredEnvironmentMapCM);
	for (unsigned int i = 0; i < 6; i++)
	{
	...
	glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
	glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_R, GL_CLAMP_TO_EDGE);
	glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR); //scrolling down - for trilinear filtering
	glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_MAG_FILTER, GL_LINEAR); //scrolling up
	glGenerateMipmap(GL_TEXTURE_CUBE_MAP); //for storing sequencially rougher reflections in the prefiltered  map's mipmap levels
	glBindTexture(GL_TEXTURE_CUBE_MAP, 0); //unbind cubemap

	//capturing prefiltered mipmap levels
	prefilteredShader.use();
	prefilteredShader.setInt("u_sampler_environment_map", 0);
	prefilteredShader.setMat4("u_perspective_matrix", gCaptureProjection);
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_CUBE_MAP, envCubeMapCM);
	...
	for (unsigned int mip = 0; mip < maxMipLevels; mip++)
	{
		...
		for (unsigned int i = 0; i < 6; i++)
		{
			prefilteredShader.setMat4("u_view_matrix", gCaptureViews[i]);
			glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, prefilteredEnvironmentMapCM, mip);
			glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
			//draw cube
			glBindVertexArray(cubeVAO);
			glDrawArrays(GL_TRIANGLES, 0, 36);
