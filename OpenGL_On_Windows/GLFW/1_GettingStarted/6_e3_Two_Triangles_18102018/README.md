## 🌱 Description
The program is a demonstration of how to render two triangles with different colors in OpenGL using seperate VAO (Vertext Buffer Object) for each triangle. Two shader programs are used and two fragment shaders. Each fragment shader has hardcoded values of colors which each triangle renders. GLFW library is used for the window management and OpenGL context management. To generate binary .exe file, follow [below steps](#how-to-run). Once .exe file is  executed, a new window will appear with an output rendered on it.
