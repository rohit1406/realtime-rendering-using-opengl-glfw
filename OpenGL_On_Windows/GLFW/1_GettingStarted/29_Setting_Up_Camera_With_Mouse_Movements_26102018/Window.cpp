...
float yaw = -90.0f; //since yaw of 0.0 results in a direction vector pointing to the right so we initially rotate a bit to left
float pitch = 0.0f;
	...
	while (!glfwWindowShouldClose(window)) //check if GLFW has been instructed to close
	{
		...
		*/
		...
		gViewMatrix = glm::lookAt(
			...
			cameraPos + cameraTarget, //where camera is looking at
			cameraUp //up vector of the camera
						);
...
void processInput(GLFWwindow* window)
{
	...
	{
		cameraPos = cameraPos + cameraSpeed * cameraTarget;
	}
	//come backward
	...
	{
		cameraPos = cameraPos - glm::normalize(glm::cross(cameraTarget, cameraUp)) * cameraSpeed;
	}
	//go right
