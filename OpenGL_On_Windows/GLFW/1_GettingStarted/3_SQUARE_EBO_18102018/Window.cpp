/*
	...
	Program Name - Hello Triangle using OpenGL
...
int main()
{
	...
	//indices that OpenGL uses to decide what vertices to draw - indexed drawing
	...
	//bind the EBO
	glBindBuffer(
			GL_ELEMENT_ARRAY_BUFFER, //assign type to created buffer on GPU
			EBO //created buffer
			);
	//copy indices to this buffer's memory
	glBufferData(
			GL_ELEMENT_ARRAY_BUFFER, //this buffer is your target
			sizeof(indices), //size of data which will be copied
			indices, //actual data
			GL_STATIC_DRAW
			);
	...
	//stored in the VAO; keeps the EBO bound
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);

	//draw in wireframe mode
	glPolygonMode(
			GL_FRONT_AND_BACK, //we want to apply it to front and back of all triangles
			GL_LINE //draw them as lines; GL_FILL is the default mode
			);
	...
	while (!glfwWindowShouldClose(window)) //check if GLFW has been instructed to close
	{
		...
		//we want to render the triangles from the index buffer; using indices from element buffer object
		glDrawElements(GL_TRIANGLES, //we are drawing triangles
