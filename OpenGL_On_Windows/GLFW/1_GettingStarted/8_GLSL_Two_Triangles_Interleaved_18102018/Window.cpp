/*
	...
	Also, color dicision is made in vertex shader itself and passed it to fragment shader.

	We are passing not only position data but also color values also as attribute in vertex data
...
int main()
{
	...
	int vertexColorUniformLocation = glGetUniformLocation(shaderProgram_yellow, //uniform is in this shaderProgram object
										"ourColor" //name of the uniform variable
										);
	//Input Vertex Data
	...
	glVertexAttribPointer(
					...
					2 * 3 * sizeof(float), //space between consecutive vertex attribtes - stride
	...
	while (!glfwWindowShouldClose(window)) //check if GLFW has been instructed to close
	{
		...
		//create changging color for second triangle
		float timeValue = glfwGetTime();
		float greenValue = (sin(timeValue) / 2.0f) + 0.5f;
		//set uniform value
		glUniform4f(vertexColorUniformLocation, //location of the uniform in shader program object
			0.0f, greenValue, 0.0f, 1.0f //color sent to the uniform
				);
