/*
	...
	Program Name - Advanced OpenGL - Blending
	...
	Stencil buffer usually contains 8 bits per stencil value that amounts to a total of
	256 values per pixel/fragment.
*/
...
int main()
{
	...
	Shader ourShader_outlined("vertex_shader.vs", "fragment_shader_outline.fs");
	...
	//set bit mask that is ANDed with the stencil value
	//0xFF means each bit is written to stencil buffer and 0x00 means disable writes to stencil buffer
	glStencilMask(0xFF);

	//configure WHEN stencil test should affect stencil buffer
	glStencilFunc(
		GL_ALWAYS, //sets the stencil test function
		1, //reference value for stencil test. The stencil buffer's content is compared with this value
		0xFF //mask ANDed with ref value and stored stencil value before stencil test happens
	);

	//tell HOW the stencil buffer should be updated
	glStencilOp(
		GL_KEEP, //sfail: action to take if stencil test fails
		GL_KEEP, //dpfail: action to take if stencil test passes but depth test fails
		GL_KEEP //dppass: action to take if both stencil test and depth test passes
	);
	...
	while (!glfwWindowShouldClose(window)) //check if GLFW has been instructed to close
	{
		...
		//set action to take when stencil test pass/fails
		glStencilOp(
			GL_KEEP, //when stencil test fails, do nothing, keep the currently stored value that is in the stencil buffer
			GL_KEEP, //action when depth test fails and stencil test pass, do nothing, keep the currently stored value that is in the stencil buffer
			GL_REPLACE //action when depth and stencil test passes, replace the reference value(which we set in glStencilFunc) with the value stored in the stencil buffer
		);
		...
		glClear(GL_COLOR_BUFFER_BIT //clear color buffer
				...
				| GL_STENCIL_BUFFER_BIT //clear stencil buffer - set all the values to zero
				);
		...
		glStencilMask(0x00); //disable writing to stencil buffer while drawing the floor
		...
		/*******************Draw Container Starts****************************/

		//set when the stencil test should pass
		glStencilFunc(
			GL_ALWAYS, //always pass the test - update each of the fragments stencil value with reference value, 1
			1, //when passed, store the reference value as 1
			0xFF //mask the reference value with 1 i.e. keep the value as it is
		);
		glStencilMask(0xFF); //enable writing to stencil buffer - while drawing the two containers

		//draw our first container
		...
		/*****************Draw Upscaled Containers now********************/
		//set when the stencil test should pass
		glStencilFunc(
			GL_NOTEQUAL, //we are only drawing part of the container which are not equal to 1
			1, //when passed, store the reference value as 1
			0xFF //mask the reference value with 1 i.e. keep the value as it is
		);
		glStencilMask(0x00); //disable writing to stencil buffer - while drawing scaled up containers

		glDisable(GL_DEPTH_TEST); //disable depth test

		ourShader_outlined.use();
		float scale = 1.1f;

		//pass uniform values
		ourShader_outlined.setMat4("u_view_matrix", gViewMatrix);
		ourShader_outlined.setMat4("u_perspective_matrix", gPerspectiveProjectionMatrix);

		//bind which VAO we are going to use with the program object
		glBindVertexArray(VAO_Cube);
		//perform model transformation
		gModelMatrix = glm::mat4(); //set to identity
		translate_object_vec = glm::vec3(-1.0f, 0.0f, -1.0f);
		gModelMatrix = glm::translate(gModelMatrix, //model matrix
			translate_object_vec //translation vector
		);

		//scale the container
		gModelMatrix = glm::scale(gModelMatrix, //target
			glm::vec3(scale, scale, scale) //scale vector
		);

		//pass uniforms
		ourShader_outlined.setMat4("u_model_matrix", gModelMatrix);
		...
		glBindVertexArray(0); //unbind the VAO
		/*****************Draw Upscaled Containers Ends********************/

		//restore the stencil mask
