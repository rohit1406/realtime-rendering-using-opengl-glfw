/*
	...
	Program Name - Advanced OpenGL - Geometry Shader
	1. Visualize Normal Vectors
...
int main()
{
	...
	Shader ourShaderWithoutNormal("vertex_shader_withoutNormal.vs", "fragment_shader_withoutNormal.fs");
	Shader ourShaderWithNormal("vertex_shader.vs", "fragment_shader.fs", "geometry_shader.gs");
	...
	glGenVertexArrays(1,  //how many vertex array objects you want to create
					&VAO_Points //id of vertex array object is store in this
				);
	...
	glGenBuffers(
			...
			&VBO_Points //this will contain the buffer id of the created buffer on GPU
			);
	...
	glVertexAttribPointer(
		...
		GL_FLOAT, //type of the data
		GL_FALSE, //do you want the data to be normalized (in -1 to 1 range)
		6 * sizeof(GL_FLOAT), //space between consecutive vertex attribtes - stride
	...
	while (!glfwWindowShouldClose(window)) //check if GLFW has been instructed to close
	{
		...
		//pass uniforms
			ourShaderWithoutNormal.setMat4("u_model_matrix", gModelMatrix);
			ourShaderWithoutNormal.setMat4("u_view_matrix", gViewMatrix);
			ourShaderWithoutNormal.setMat4("u_perspective_matrix", gPerspectiveProjectionMatrix);
		
		//draw object
		ourModel.Draw(ourShaderWithoutNormal);


		/*******Draw With Normal********/
		ourShaderWithNormal.use();
		gModelMatrix = glm::mat4();
		gModelMatrix = glm::translate(gModelMatrix,
			translate_object_vec //translate by
		);
		...
		//pass uniforms
		ourShaderWithNormal.setMat4("u_model_matrix", gModelMatrix);
		ourShaderWithNormal.setMat4("u_view_matrix", gViewMatrix);
		ourShaderWithNormal.setMat4("u_perspective_matrix", gPerspectiveProjectionMatrix);

		//draw object
		ourModel.Draw(ourShaderWithNormal);
	...
	glDeleteVertexArrays(1, &VAO_Points); //points vao
	glDeleteBuffers(1, &VBO_Points); //points vbo
