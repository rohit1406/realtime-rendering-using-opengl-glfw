/*
	...
	Program Name - Advanced OpenGL - Framebuffers/ off-screen rendering / Rendering to texture
	We're going to render the scene into a color texture attached to a framebuffer object we created 
	and then draw this texture over a simple quad that spans the whole screen. 
	The visual output is then exactly the same as without a framebuffer, but this time it's 
	all printed on top of a single quad.

	Post Processing Effects - Blur Effect
	A kernel (or convolution matrix) is a small matrix-like array of values centered on 
	the current pixel that multiplies surrounding pixel values by its kernel values and 
	adds them all together to form a single value. So basically we're adding a small offset 
	to the texture coordinates in surrounding directions of the current pixel and combine 
	the results based on the kernel.

	This kernel takes 8 surrounding pixel values and multiplies them by 2 and the current pixel
	by -15. This example kernel basically multiplies the surrounding pixels by a weight determined 
	in the kernel and balances the result by multiplying the current pixel by a large negative weight.

	A kernel that gives blue effect is implemented in fragment shader of a screen.
...
int main()
{
	...
	//to render screen
	Shader ourShader_screen("vertex_shader_screen.vs", "fragment_shader_screen.fs");

	//to render scene
	Shader ourShader_scene("vertex_shader_scene.vs", "fragment_shader_scene.fs");
	...
	/**************Setup FrameBuffer Start***********************/
	unsigned int FBO;
	//create framebuffer object in GPU memory
	glGenFramebuffers(
		1, //how many objects you want to create
		&FBO //ID of generated framebuffer is stored in this
	);

	//bind it as a active framebuffer
	glBindFramebuffer(
		GL_FRAMEBUFFER, //target - the created framebuffer is of this type
		FBO //id of the framebuffer
	);

	//create the attachment for the framebuffer
	//we can create attachment as a texture or render buffers
	unsigned int textureColorBuffer;
	//create texture in memory
	glGenTextures(1, //how many
			&textureColorBuffer //id of the generated texture will be stored in this
		);

	//bind the texture to make it as active
	glBindTexture(GL_TEXTURE_2D, //target
		textureColorBuffer //mark texture with this id as GL_TEXTURE_2D
		);

	//generate texture - we are only allocating memory but not filling it
	//filling the texture will happen as soon as we render to the framebuffer
	glTexImage2D(
		GL_TEXTURE_2D, //target
		0, //mipmap level
		GL_RGB, //internal format - data in this texture will be stored in this format
		WIN_WIDTH, //width
		WIN_HEIGHT ,//height
		0, //border
		GL_RGB, //format of data from which we are creating this texture
		GL_UNSIGNED_BYTE, //type of data
		NULL //actual data
	);

	glTexParameteri(GL_TEXTURE_2D, //target
		GL_TEXTURE_MIN_FILTER, //when scroll down
		GL_LINEAR //sample taken from neighbouring texels
		);

	glTexParameteri(GL_TEXTURE_2D, //target
		GL_TEXTURE_MAG_FILTER, //when scroll up
		GL_LINEAR //sample taken from neighbouring texels
	);
	...
	glFramebufferTexture2D(
		...
		GL_COLOR_ATTACHMENT0, //attach it as a color attachment
		GL_TEXTURE_2D, //textarget - type of texture you want to attach
		textureColorBuffer, //id of the texture to be attached
		0 //mipmap level
	);

	//create depth and stencil attachment using renderbuffer object
	unsigned int rboId; //render buffer object
	...
	//attach renderbuffer object to the framebuffer
	...
	//check if framebuffer is actually complete
	...
	glBindFramebuffer(
		GL_FRAMEBUFFER, //target - the created framebuffer is of this type
		0 //unbinds the framebuffer
	);

	/**************Setup FrameBuffer Ends***********************/
	...
	/********************Setup QUAD VAO START*******************/
	//create vertex array object
	//a VAO that stores our vertex attribute configuration
	//and which VBO to use
	unsigned int VAO_Quad, VBO_Quad;
	glGenVertexArrays(1,  //how many vertex array objects you want to create
		&VAO_Quad //id of vertex array object is store in this
	);
	//bind vertex array object first, then bind and set vertex buffers
	//and then configure vertex attributes
	...
	glGenBuffers(
		1, //how many buffers you want to create
		&VBO_Quad //this will contain the buffer id of the created buffer on GPU
	);

	//here onwards, any buffer call(on GL_ARRAY_BUFFER target) will be used to configure the currently bound buffer, VBO
	glBindBuffer(
		GL_ARRAY_BUFFER, //type of created buffer on GPU
		VBO_Quad //created buffer
	);
	...
	glVertexAttribPointer(
		...
		GL_FALSE, //do you want the data to be normalized (in -1 to 1 range)
		4 * sizeof(float), //space between consecutive vertex attribtes - stride
		(void *)(2 * sizeof(float)) //from where the position data begins in the buffer
	);
	...
	while (!glfwWindowShouldClose(window)) //check if GLFW has been instructed to close
	{
		...
		//first pass
		//bind framebuffer to make it as active
		...
		ourShader_scene.setInt("u_texture0",0);
		...
		//pass uniforms
		ourShader_scene.setMat4("u_model_matrix", gModelMatrix);
		ourShader_scene.setMat4("u_view_matrix", gViewMatrix);
		ourShader_scene.setMat4("u_perspective_matrix", gPerspectiveProjectionMatrix);
		...
		//clear the screen with color of your choice
		//else we should see the results from previous iteration
		//entire color buffer will be filled with this color
		...
		//use screen shader
		ourShader_screen.use();

		//pass texture sampler unit
		ourShader_screen.setInt("u_screen_texture", 0);
		ourShader_screen.setMat4("u_view_matrix", gViewMatrix);
		ourShader_screen.setMat4("u_perspective_matrix", gPerspectiveProjectionMatrix);
		//bind which VAO we are going to use with the program object - here drawing floor
		glBindVertexArray(VAO_Quad);
		...
		//pass uniforms
		ourShader_screen.setMat4("u_model_matrix", gModelMatrix);
		...
		glBindTexture(
			GL_TEXTURE_2D, //texture target
			textureColorBuffer //use color attachment as a texture of the quad plane
		);
	...
	glDeleteVertexArrays(1, &VAO_Quad); //Quad vao
	glDeleteBuffers(1, &VBO_Cube); //cube vbo
	glDeleteBuffers(1, &VBO_Plane); //plane vbo
	glDeleteBuffers(1, &VBO_Quad); //quad vbo
