/*
	...
	Environment Mappings - Refraction
...
int main()
{
	...
	while (!glfwWindowShouldClose(window)) //check if GLFW has been instructed to close
	{
		...
		ourShader.setInt("u_skybox_sampler",0);
		...
		glm::vec3 translate_object_vec = glm::vec3(0.0f, -0.5f, -0.3f);
		...
		gModelMatrix = glm::rotate(gModelMatrix, //model matrix
			glm::radians(30.0f), //rotation angle
			glm::vec3(0.0f, 0.5f, 1.0f) //rotation axis vector
		);
