/*
	...
	Program Name - Advanced OpenGL - gl_FrontFacing
	Used Interface blocks in shaders and used uniform buffer object (UBO) to share global data between multiple shader objects
...
int main()
{
	...
	Shader redShader("vertex_shader.vs", "red.fs");
	Shader greenShader("vertex_shader.vs", "green.fs");
	Shader blueShader("vertex_shader.vs", "blue.fs");
	Shader yellowShader("vertex_shader.vs", "yellow.fs");
	...
	*/
	...
	/**********Set Uniform Blocks Start*****************/
	//get id of the uniform block
	unsigned int uniformBlockIndexRed = glGetUniformBlockIndex(redShader.ID, "u_transformation_matrices");
	unsigned int uniformBlockIndexGreen = glGetUniformBlockIndex(greenShader.ID, "u_transformation_matrices");
	unsigned int uniformBlockIndexBlue = glGetUniformBlockIndex(blueShader.ID, "u_transformation_matrices");
	unsigned int uniformBlockIndexYellow = glGetUniformBlockIndex(yellowShader.ID, "u_transformation_matrices");
	
	//bind the uniform block to the binding
	glUniformBlockBinding(redShader.ID,  //shader program object
		uniformBlockIndexRed, //uniform block index
		0 //uniform block binding
	);
	glUniformBlockBinding(greenShader.ID,  //shader program object
		uniformBlockIndexGreen, //uniform block index
		0 //uniform block binding
	);
	glUniformBlockBinding(blueShader.ID,  //shader program object
		uniformBlockIndexBlue, //uniform block index
		0 //uniform block binding
	);
	glUniformBlockBinding(yellowShader.ID,  //shader program object
		uniformBlockIndexYellow, //uniform block index
		0 //uniform block binding
	);

	//create the actual uniform buffer object
	...
	glBufferData(
		GL_UNIFORM_BUFFER, //target
		2 * sizeof(glm::mat4), //size of the memory to allocate
		NULL, //passing actual data as null
		GL_STATIC_DRAW //data won't change so frquently
	);

	//unbind the buffer
	glBindBuffer(GL_UNIFORM_BUFFER, 0);

	//link uniform buffer to the same binding point
	glBindBufferRange(
		GL_UNIFORM_BUFFER, //target
		0, //index
		UBO_Matrices, //buffer
	...
	//calculate and store the matrices in uniform buffer
	gPerspectiveProjectionMatrix = glm::mat4(); //set to identity
	...
	glBindBuffer(GL_UNIFORM_BUFFER, UBO_Matrices); //bind UBO
	glBufferSubData(GL_UNIFORM_BUFFER, //target
		0, //starting off-set
		sizeof(glm::mat4), //size of data
		glm::value_ptr(gPerspectiveProjectionMatrix) //actual data
	);
	glBindBuffer(GL_UNIFORM_BUFFER, 0); //unbind UBO

	/**********Set Uniform Blocks end*****************/
	...
	while (!glfwWindowShouldClose(window)) //check if GLFW has been instructed to close
	{
		...
		//view transformation
		gViewMatrix = glm::mat4(); //set to identity
		gViewMatrix = camera.GetViewMatrix();
		glBindBuffer(GL_UNIFORM_BUFFER, UBO_Matrices); //bind UBO
		glBufferSubData(GL_UNIFORM_BUFFER, //target
			sizeof(glm::mat4), //starting off-set
			sizeof(glm::mat4), //size of data
			glm::value_ptr(gViewMatrix) //actual data
		);
		glBindBuffer(GL_UNIFORM_BUFFER, 0); //unbind UBO
		...
		redShader.setInt("u_texture0",0);
		redShader.setInt("u_texture1", 1);
		/**********CREATE TRANSFORMATION MATRICES***************/
		//set transformation matrices to identity matrix
		gModelMatrix = glm::mat4();
		
		//model transformation
		glm::vec3 translate_object_vec = glm::vec3(-0.75f, 0.75f, 0.0f); //top-left
		gModelMatrix = glm::translate(gModelMatrix, //model matrix
			translate_object_vec //translation vector
		);

		//pass uniforms
		redShader.setMat4("u_model_matrix", gModelMatrix);
		...
		glActiveTexture(GL_TEXTURE1); //1st texture unit
		...
		greenShader.use();
		//set sampler - texture unit
		greenShader.setInt("u_texture0", 0);
		greenShader.setInt("u_texture1", 1);
		/**********CREATE TRANSFORMATION MATRICES***************/
		//set transformation matrices to identity matrix
		gModelMatrix = glm::mat4();

		//model transformation
		translate_object_vec = glm::vec3(0.75f, 0.75f, 0.0f); //top-right
		gModelMatrix = glm::translate(gModelMatrix, //model matrix
			translate_object_vec //translation vector
		);

		//pass uniforms
		greenShader.setMat4("u_model_matrix", gModelMatrix);
		//bind which VAO we are going to use with the program object
		glBindVertexArray(VAO_Cube);
		...
		blueShader.use();
		//set sampler - texture unit
		blueShader.setInt("u_texture0", 0);
		blueShader.setInt("u_texture1", 1);
		/**********CREATE TRANSFORMATION MATRICES***************/
		//set transformation matrices to identity matrix
		gModelMatrix = glm::mat4();

		//model transformation
		translate_object_vec = glm::vec3(-0.75f, -0.75f, 0.0f); //bottom-left
		gModelMatrix = glm::translate(gModelMatrix, //model matrix
			translate_object_vec //translation vector
		);

		//pass uniforms
		blueShader.setMat4("u_model_matrix", gModelMatrix);
		//bind which VAO we are going to use with the program object
		glBindVertexArray(VAO_Cube);
		...
		yellowShader.use();
		//set sampler - texture unit
		yellowShader.setInt("u_texture0", 0);
		yellowShader.setInt("u_texture1", 1);
		/**********CREATE TRANSFORMATION MATRICES***************/
		//set transformation matrices to identity matrix
		gModelMatrix = glm::mat4();

		//model transformation
		translate_object_vec = glm::vec3(0.75f, -0.75f, 0.0f); //top-left
		gModelMatrix = glm::translate(gModelMatrix, //model matrix
			translate_object_vec //translation vector
		);

		//pass uniforms
		yellowShader.setMat4("u_model_matrix", gModelMatrix);
		//bind which VAO we are going to use with the program object
		glBindVertexArray(VAO_Cube);
