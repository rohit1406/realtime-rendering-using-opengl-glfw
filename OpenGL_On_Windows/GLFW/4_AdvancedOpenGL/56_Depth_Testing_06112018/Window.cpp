/*
	...
	In this program, we have set the depth func as GL_ALWAYS this makes the depth test to pass always and
	so the fragments that are drawn last are rendered in the front of the fragments that were drawn before, even though they shouldn't have to be at the front.
	Since we have drawn the floor plane as last, the planes fragment overrite each of the container's fragments

	This is good exmaple to understand Depth Testing.
...
int main()
{
	...
	//so the fragments that are drawn last are rendered in front of the fragments that were drawn before, even though they should not have been at the front
