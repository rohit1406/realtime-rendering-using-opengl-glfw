/*
	...
	Program Name - Advanced OpenGL - Indexed Drawing with Instanced Arrays
	Try commenting pos from vertex shader
...
int main()
{
	...
	//Shader ourShader("vertex_shader.vs", "fragment_shader.fs", "geometry_shader.gs");
	...
	//create VBO for instanced arrays
	...
	glBindBuffer(
		...
		VBO_Instanced //buffer to bound
	);

	//add data to buffer
	glBufferData(
		GL_ARRAY_BUFFER, //target
		sizeof(glm::vec2) * 100, //size of data
		&translations[0], //actual data
		GL_STATIC_DRAW
	);
	...
	glVertexAttribPointer(
		2, //attribute index
		2, //one attribute contains 2 floats
		GL_FLOAT, //data type of data
	...
	//By setting the attribute divisor to 1 we're effectively telling OpenGL that the 
	...
	glVertexAttribDivisor(2, //attribute index
		1 //divisor - 0 means update element to each iteration of the vertex shader
		//1 means update when we start to render new instance
		//2 mean every two instances
		);
	...
	while (!glfwWindowShouldClose(window)) //check if GLFW has been instructed to close
	{
		...
		glm::vec3 translate_object_vec = glm::vec3(0.0f, 0.0f, 0.0f);
	...
	glDeleteVertexArrays(1, &VAO_Quad); //points vao
	glDeleteBuffers(1, &VBO_Quad); //points vbo
