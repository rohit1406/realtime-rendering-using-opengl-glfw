/*
	...
	Program Name - Advanced OpenGL - Anti-Aliasing
...
int main()
{
	...
	//hint window to provide multisampled buffers
	glfwWindowHint(GLFW_SAMPLES, 4); //4 samples per pixel
	...
	while (!glfwWindowShouldClose(window)) //check if GLFW has been instructed to close
	{
		...
		gModelMatrix = glm::rotate(gModelMatrix, //model matrix
			...
			glm::vec3(1.0f, 1.0f, 1.0f) //translation vector
		);
