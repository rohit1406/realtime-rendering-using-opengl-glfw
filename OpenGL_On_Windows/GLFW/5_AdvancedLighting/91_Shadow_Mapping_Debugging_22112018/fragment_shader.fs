...
uniform sampler2D u_sampler_texture0;
uniform bool blinn;

void main()
{
	vec3 color = texture(u_sampler_texture0, fs_in.out_texcoords).rgb;

//ambient	
float ambient_factor = 0.05; //a constant ambient factor
...
vec3 specular_lighting = vec3(0.3) * spec;
