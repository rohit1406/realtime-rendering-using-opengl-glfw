/*
	...
	D3000 nicon
	Program Name - High Dynamic Range
	The process of transforming floating point color values (high dynamic range) to the expected 0,1 range (low dynamic range)
	without losing too much details is called tone mapping.
...
//camera
Camera camera(glm::vec3(0.0f, 0.0f, 20.0f) //position
				);
//mouse movements
...
int main()
{
	...
	//lighting positions
	gLightPositions.push_back(glm::vec3(0.0f, 0.0f, 49.5f)); //back light
	gLightPositions.push_back(glm::vec3(-1.4f, -1.9f, 9.0f));
	gLightPositions.push_back(glm::vec3(0.0f, -1.8f, 4.0f));
	gLightPositions.push_back(glm::vec3(0.8f, -1.7f, 6.0f));

	//lighting colors
	gLightColors.push_back(glm::vec3(200.0f, 200.0f, 200.0f));
	gLightColors.push_back(glm::vec3(0.1f, 0.0f, 0.0f));
	gLightColors.push_back(glm::vec3(0.0f, 0.0f, 0.2f));
	gLightColors.push_back(glm::vec3(0.0f, 0.1f, 0.0f));

	lighting_shader.use();
	lighting_shader.setInt("u_sampler_diffuse_texture", 0);
	...
	while (!glfwWindowShouldClose(window)) //check if GLFW has been instructed to close
	{
		...
		*/
		...
		{
			lighting_shader.setVec3("lights["+std::to_string(i)+"].Position", gLightPositions[i]);
			lighting_shader.setVec3("lights[" + std::to_string(i) + "].Color", gLightColors[i]);
		}
		...
		//draw cubes with different model matrices
			...
			gModelMatrix = glm::translate(gModelMatrix, glm::vec3(0.0f, 0.0f, 25.0f)); //translate
			gModelMatrix = glm::scale(gModelMatrix, glm::vec3(2.5f, 2.5f, 27.5f)); //scale
			
			lighting_shader.setMat4("u_model_matrix", gModelMatrix);
			lighting_shader.setInt("u_inverse_normals", true);
			//draw cube
			glDrawArrays(
		...
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, colorBuffer);
		hdrshader.setInt("u_hdr", gbHdr);
		hdrshader.setFloat("u_exposure", gfExposure);
		hdrshader.setInt("u_sampler_hdr_color", 0);
		//bind vao
		glBindVertexArray(vao_quad);
		//draw cube
		glDrawArrays(
...
void processInput(GLFWwindow* window)
{
	...
	//hdr
	if (glfwGetKey(window, GLFW_KEY_SPACE) == GLFW_PRESS && !gbHdr)
	{
		gbHdr = !gbHdr;
		gbHdr = true;
	}
