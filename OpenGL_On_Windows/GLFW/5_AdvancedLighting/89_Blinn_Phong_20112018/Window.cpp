/*
	...
	Program Name - a lighting scene - Phong and Blinn-Phong modifications
...
int main()
{
	...
	while (!glfwWindowShouldClose(window)) //check if GLFW has been instructed to close
	{
		...
		//set sampler
		lighting_shader.setInt("u_sampler_texture0", 0); //texture unit
		...
		*/
		...
		//draw model with different model matrices
		...
		lighting_shader.setBool("blinn", isBPressed);
