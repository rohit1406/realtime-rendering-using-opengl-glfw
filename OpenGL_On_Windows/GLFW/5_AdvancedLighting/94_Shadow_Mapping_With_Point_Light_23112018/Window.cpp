...
#Not Working
Program Name - a lighting scene - Shadow Mapping - point light - Omnidirectional Shadow Mapping
Covers Basics of Shadow Mapping - output removed shadow acne from previous tutorial
Important Notes:
...
int main()
{
	...
	Shader simple_depth_shader("simple_depth_shader.vs", "simple_depth_shader.fs", "simple_depth_shader.gs");
	...
	glGenTextures(1, &depthCubeMap); //this creates texture buffer in memory

	//bind this texture to 2D texture target
	glBindTexture(GL_TEXTURE_CUBE_MAP, //target
		depthCubeMap //texture to bind
	);
	...
	glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_MIN_FILTER, GL_NEAREST); //when downscaling
	glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_MAG_FILTER, GL_NEAREST); //when upscaling
	glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE); //s texcoord
	glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE); //t texcoord
	glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_R, GL_CLAMP_TO_EDGE); //r texcoord
	...
	while (!glfwWindowShouldClose(window)) //check if GLFW has been instructed to close
	{
		...
		for (int i = 0; i < 6; ++i)
		{
			simple_depth_shader.setMat4("u_shadow_matrices["+std::to_string(i)+"]", shadowTransforms[i]);
		}
		simple_depth_shader.setFloat("u_far_plane",far_plane);
		simple_depth_shader.setVec3("u_light_pos", lightPos);
		//bind framebuffer
		glBindFramebuffer(GL_FRAMEBUFFER, depthMapFBO);
		...
		//reset face culling to original
		glCullFace(GL_BACK);
