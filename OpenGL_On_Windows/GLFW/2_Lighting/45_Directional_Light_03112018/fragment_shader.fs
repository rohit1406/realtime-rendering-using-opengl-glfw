...
struct Light
{
//vec3 position; //no longer necessary when using directional light
vec3 direction;
vec3 ambient;
vec3 diffuse;
vec3 specular;
...
vec3 light_direction = normalize(-light.direction); //light's direction vector pointing towards the light source
