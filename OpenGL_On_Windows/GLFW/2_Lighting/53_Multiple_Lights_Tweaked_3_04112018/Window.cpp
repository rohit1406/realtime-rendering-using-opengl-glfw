...
int main()
{
	...
	//diffuse map
	unsigned int diffuseMap = loadTexture("IMG2.jpg");
	...
	while (!glfwWindowShouldClose(window)) //check if GLFW has been instructed to close
	{
		...
		*/
		...
		for (unsigned int i = 0; i < _ARRAYSIZE(cubePositions); i++)
		{
			...
			float angle = glfwGetTime() * i * 20.0f;
		...
		//zeroth light source
			gModelMatrix = glm::mat4(); //set to identity
			gModelMatrix = glm::translate(gModelMatrix, pointLightPositions[0]);
			gModelMatrix = glm::scale(gModelMatrix, glm::vec3(0.2f));
			light_color = glm::vec3(0.0f,0.0f, 1.0f); //blue
			lamp_shader.setVec3("u_light_color_lp", light_color);
		...
		//first light source
			gModelMatrix = glm::mat4(); //set to identity
			gModelMatrix = glm::translate(gModelMatrix, pointLightPositions[1]);
			gModelMatrix = glm::scale(gModelMatrix, glm::vec3(0.2f));
			light_color = glm::vec3(0.0f, 1.0f, 0.0f); //green
			lamp_shader.setVec3("u_light_color_lp", light_color);
			lamp_shader.setMat4("u_model_matrix", gModelMatrix);
		...
		//second light source
			gModelMatrix = glm::mat4(); //set to identity
			gModelMatrix = glm::translate(gModelMatrix, pointLightPositions[2]);
			gModelMatrix = glm::scale(gModelMatrix, glm::vec3(0.2f));
			light_color = glm::vec3(1.0f, 0.0f, 0.0f); //red
			lamp_shader.setVec3("u_light_color_lp", light_color);
			lamp_shader.setMat4("u_model_matrix", gModelMatrix);
			...
			//third light source
			gModelMatrix = glm::mat4(); //set to identity
			gModelMatrix = glm::translate(gModelMatrix, pointLightPositions[3]);
			gModelMatrix = glm::scale(gModelMatrix, glm::vec3(0.2f));
			light_color = glm::vec3(0.0f, 1.0f, 1.0f); //different color
			lamp_shader.setVec3("u_light_color_lp", light_color);
			lamp_shader.setMat4("u_model_matrix", gModelMatrix);
