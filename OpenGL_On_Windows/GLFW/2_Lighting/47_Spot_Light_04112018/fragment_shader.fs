...
//negating because we want to point towards light source instead of from
float theta = dot(light_direction, normalize(-light.direction));

//theta is angle between light direction and spot light
//cutoff is radius of the spot light
if(theta > light.cutoff) //means visible
{
//do lighting calculations
final_color = ambient_lighting + diffuse_lighting + specular_lighting;
}else
{
//use ambient lighting so that object is not completely dark
final_color = light.ambient * vec3(texture(material.diffuse, out_texcoords));
}
