/*
	...
	Program Name - a lighting scene - ambient lighting
	Moment - Done in travels while travelling from umarkhed to pune
...
int main()
{
	...
	glVertexAttribPointer(0, //0th layout location attribute in the shader
		...
		6 * sizeof(float), //space between consecutive vertex attributes - stride
	...
	while (!glfwWindowShouldClose(window)) //check if GLFW has been instructed to close
	{
		...
		*/
		...
		gLightPosition.x = 1.0f + sin(glfwGetTime()) * 2.0f;
		gLightPosition.y = sin(glfwGetTime() / 2.0f) * 1.0f;
