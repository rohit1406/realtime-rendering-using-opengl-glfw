/*
	...
	Program Name - Emission Maps
	This is just a different name for the underlying principle of textures. In lighting, we call it as Emission maps
...
int main()
{
	...
	while (!glfwWindowShouldClose(window)) //check if GLFW has been instructed to close
	{
		...
		*/
		...
		//activate texture unit - emission
		glActiveTexture(GL_TEXTURE2); //2th texture unit

									  //bind the texture now
		glBindTexture(
			GL_TEXTURE_2D, //texture target
			emissionMap //texture value
		);
		...
		for (unsigned int i = 0; i < 10; i++)
		{
			...
			gModelMatrix = glm::rotate(gModelMatrix, //model matrix
				...
				glm::vec3(1.0f, 0.0f, 0.0f) //rotate around all axis
			);

			//roate around y axis
			gModelMatrix = glm::rotate(gModelMatrix, //model matrix
				glm::radians(angle), //get time values
				glm::vec3(0.0f, 1.0f, 0.0f) //rotate around all axis
			);

			//roate around z axis
			gModelMatrix = glm::rotate(gModelMatrix, //model matrix
				glm::radians(angle), //get time values
				glm::vec3(0.0f, 0.0f, 1.0f) //rotate around all axis
			);
