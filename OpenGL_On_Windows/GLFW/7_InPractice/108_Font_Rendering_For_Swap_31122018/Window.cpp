...
//camera
Camera camera(glm::vec3(0.0f, 0.0f, 3.0) //position
				);
//mouse movements
...
GLuint gQuadForSwapVAO, gQuadForSwapVBO;
GLuint gSwapTBO;
...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(
									WIN_WIDTH, //width
									WIN_HEIGHT, //height
									"OpenGL: 2019 Gift To My Life", //title
									nullptr, //monitor
									NULL //share
								);
	...
	Shader swapShader("swap.vs", "swap.fs");
	...
	*/
	...
	//load Swapnali's image
	//textures
	gSwapTBO = loadTexture("resources/textures/swap.JPG", true);
	...
	*/
	...
	glGenVertexArrays(1,  //how many vertex array objects you want to create
		&gQuadForSwapVAO //id of vertex array object is store in this
	);
	//bind vertex array object first, then bind and set vertex buffers
	//and then configure vertex attributes
	glBindVertexArray(gQuadForSwapVAO);

	//create memory on GPU - vertex buffer object
	glGenBuffers(
		1, //how many buffers you want to create
		&gQuadForSwapVBO //this will contain the buffer id of the created buffer on GPU
	);

	//here onwards, any buffer call(on GL_ARRAY_BUFFER target) will be used to configure the currently bound buffer, VBO
	glBindBuffer(
		GL_ARRAY_BUFFER, //type of created buffer on GPU
		gQuadForSwapVBO //created buffer
	);
	...
	//you can unbind the vao afterwards so that other VAO calls won't accidentaly modify this VAO
	glBindVertexArray(0); //unbind swap vao
	...
	swapShader.use();
	swapShader.setInt("u_sampler_swap", 0);
	...
	while (!glfwWindowShouldClose(window)) //check if GLFW has been instructed to close
	{
		...
		glBindTexture(GL_TEXTURE_2D, gSwapTBO);
		...
		swapShader.setMat4("u_perspective_matrix", gPerspectiveProjectionMatrix);

		//view transformation
		gViewMatrix = camera.GetViewMatrix();
		swapShader.setMat4("u_view_matrix", gViewMatrix);

		//model transformation
		gModelMatrix = glm::translate(gModelMatrix, glm::vec3(0.0f, 0.0f, -1.0f));
		swapShader.setMat4("u_model_matrix", gModelMatrix);
		glBindVertexArray(gQuadForSwapVAO);
		glDrawArrays(GL_TRIANGLES, 0, 6);
		glBindVertexArray(0); //unbind swap vao
		//render swap image ends
